...
\begin{proof}
	...
	$u_n\to u:\ \forall  ε>0\ \exists N$ t.q. $\forall n≥N |u_n-u|≤ε$. On choisit $ε<|u-u'|$ alors on a toujours si $n≥N$ $|u_n-u'|=|u_n-u+u-u'|=||u-u'|-|u_n-u|| ≥|u-u'|-ε≥\frao{|u-u'|}2$ => $\forall n≥N |u_n-u'|≥\frac{|u-u'|}{2}$ => $|u_n-u'|\not\to 0$ <=> $u_n\not\to u'$ QED.
\end{proof}
...
\end{proposition}
\begin{remark}
	Si $(x_n)_{n\in\N}$ converge $\esixts x\in H$ et $\lim_{n\to +∞}\norm{x_n-x}=0$ alors par $|\norm{x}-\norm{x_n}|≤\norm{x-x_n}$ => \lim_{n\to ∞}\norm{x_n}=\norm{x}.
...
\begin{propositon}
	Soit $(x_n)_{n\in\N}$ une suite dans $H$. Alors 
		$\item x_n\to x$ <=> $x_n\rightharpoonup x$ et $\lim_n\sup\norm{x_n}≤\norm{x}$
\end{propositon}
\begin{proof}
